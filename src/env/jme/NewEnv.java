...
/**
 * Class assembling the NewEnv with every objects composing the world (terrain,
 * players,..).
 * 
...
public class NewEnv extends SimpleApplication {
	...
	public void simpleInitApp() {
		...
		cam.lookAtDirection(new Vector3f(-0.0016761336f, -0.9035275f, -0.42852688f),
				new Vector3f(-0.003530928f, 0.4285301f, -0.9035206f));
	...
	public void makeTerrain() {
		...
		int patchSize = 65;
		terrain = new TerrainQuad("my terrain", patchSize, this.heightmap_tuplet.getFirst() + 1,
				this.heightmap_tuplet.getSecond());
	...
	private void initKeys() {
		...
		// this.inputManager.addMapping("FLYCAM_RotateDrag",new
		// MouseButtonTrigger(MouseInput.BUTTON_RIGHT));

		this.inputManager.addMapping("FLYCAM_StrafeLeft", new KeyTrigger(KeyInput.KEY_J));
		this.inputManager.addMapping("FLYCAM_StrafeRight", new KeyTrigger(KeyInput.KEY_L));
		this.inputManager.addMapping("FLYCAM_Forward", new KeyTrigger(KeyInput.KEY_P));
		this.inputManager.addMapping("FLYCAM_Backward", new KeyTrigger(KeyInput.KEY_M));
		this.inputManager.addMapping("FLYCAM_Rise", new KeyTrigger(KeyInput.KEY_I));
		this.inputManager.addMapping("FLYCAM_Lower", new KeyTrigger(KeyInput.KEY_K));

		// Picking association
		// this.inputManager.addMapping("Picking", new
		// MouseButtonTrigger(MouseInput.BUTTON_LEFT)); // trigger 2: left-button click
	...
	public synchronized boolean deployAgent(String agentName, String playertype, boolean color) {
		if (this.players.containsKey(agentName)) {
			...
			Box b = new Box(2, 2, 2); // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ HERE WE CAN DO
										// SOMETHING TO VISUALIZE ORIENTATION @@@@@@@@@@@@@@@@
			Geometry player = new Geometry("Box", b);
	...
	public synchronized boolean randomMove(String agent) { // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
															// SHOULD BE MODIFIED TO ALLOW RANGE SETTING @@@@@@@@@@@@@
		if (players.containsKey(agent)) {
				...
				if (r.nextFloat() < impact) {
					...
					if (enemyAgent.life <= 0) {
						...
						if (!enemy.equals("Player1")) {
							...
							thisAgent.saveVictory();
							enemyAgent.saveDefeat();
						}
	...
	public static void saveCSV() {
		...
		try {
			PrintWriter writer = new PrintWriter(
					System.getProperty("user.dir") + "/ressources/simus/Mosimu_" + id + ".csv", "UTF-8");
			writer.println(res);
			writer.close();
	...
	public static void saveCSV(String folder, String name) {
		...
		try {
			PrintWriter writer = new PrintWriter(
					System.getProperty("user.dir") + "/ressources/learningBase/" + folder + id + "_" + name + ".csv",
					"UTF-8");
			// writer.println(res);
			writer.close();
			// System.out.println("Execution result saved in /ressources/simus/");
	...
	public synchronized boolean isVisible(String agent, String enemy, float distance) { // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
																						// TODO : CHANGE TO ANGLE + CAST
		Vector3f origin = getCurrentPosition(agent);
		Vector3f target = getCurrentPosition(enemy);
		Vector3f dir = target.subtract(origin).normalize();
		...
		if (cam.getDirection().angleBetween(dir) < AbstractAgent.VISION_ANGLE) { // in angle ??
			...
			Vector3f terrainHit = (resultsTerrain.size() == 0) ? null
					: resultsTerrain.getCollision(0).getContactPoint();
			Vector3f playerHit = (resultsPlayers.size() == 0) ? null : resultsPlayers.getCollision(0).getContactPoint();

			if (resultsPlayers.size() != 0
					&& (resultsTerrain.size() == 0 || origin.distance(playerHit) > origin.distance(terrainHit))) {
				return true;
			}
	...
	public synchronized ArrayList<Vector3f> goldenSphereCast(Spatial sp, float distance, int N, float angle) {
		return filterWithVisionAngle(sp, goldenSphereCast(sp, distance, N),
				((Camera) sp.getUserData("cam")).getDirection(), angle);
	}

	public synchronized ArrayList<Vector3f> filterWithVisionAngle(Spatial sp, ArrayList<Vector3f> in, Vector3f dir,
			float maxAngle) { // @@@@@@@@@@@@@@@@@@@@@@@@@@@ WRONGGGGGGGGGGGGGGG
