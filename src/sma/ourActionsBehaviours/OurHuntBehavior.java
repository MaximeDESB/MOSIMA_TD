...
public class OurHuntBehavior extends TickerBehaviour {
	...
	public OurHuntBehavior(Agent a, long period) {
		super(a, period);
		agent = (FinalAgent)((AbstractAgent)a);
	}
	
	public OurHuntBehavior(Agent a, long period, InterestPoint firstPoint) { // Called when getting back from Attack
		super(a, period);
		agent = (FinalAgent)((AbstractAgent)a);
		target = firstPoint;
	...
	protected void onTick() {
		...
		if (enemy != null){
			...
			OurAttack a = new OurAttack(agent, FinalAgent.PERIOD, enemy.getSecond());
		...
		if (target == null){
			...
			if (point != null){
				...
				OurExploreBehavior ex = new OurExploreBehavior(agent, FinalAgent.PERIOD);
